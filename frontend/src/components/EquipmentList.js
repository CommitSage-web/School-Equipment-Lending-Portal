    ...
    <>
      <Grid container spacing={3}>
        {items.map((item, index) => {
          // resolve image source: allow full URLs, local images from src/images,
          // or fallback to a bundled image when item.image is missing
          const imgSrc = (() => {
            // name-based mapping (explicit overrides)
            const nameMap = {
            ...
            // 1) if item.image provided, prefer that (external or local filename)
            if (item.image) {
              if (/^(https?:)?\/\//i.test(item.image)) return item.image; // external URL
              const name = item.image.split("/").pop().toLowerCase();
              return images[name] || null;
            }

            // 2) if there's a name-based mapping, try to use the mapped file
            ...
            // 3) fallback: choose a bundled image deterministically by index
